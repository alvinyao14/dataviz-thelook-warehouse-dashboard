...
def render_tab(df):
    ...
    # --- Ensure datetime & derive date range ---
    ...
    lost_df["order_date"] = lost_df["created_at"].dt.date

    min_date = lost_df["order_date"].min()
    max_date = lost_df["order_date"].max()
    ...
    dc_options = sorted(lost_df["dc_name"].dropna().unique().tolist()) \
        if "dc_name" in lost_df.columns else []
    ...
    # Category filter
    cat_options = (
        sorted(lost_df["category"].dropna().unique().tolist())
    ...
    status_options = (
        sorted(lost_df["status"].dropna().unique().tolist())
    ...
    # --- KPI Cards ---
    total_lost_revenue = lost_df["sale_price"].sum() if "sale_price" in lost_df.columns else 0.0
    total_lost_margin = lost_df["gross_margin"].sum() if "gross_margin" in lost_df.columns else 0.0
    total_cost = lost_df["cost"].sum() if "cost" in lost_df.columns else 0.0
    num_leak_orders = lost_df["order_id"].nunique() if "order_id" in lost_df.columns else len(lost_df)
    ...
    # --- Lost Revenue by DC & Status (stacked bar) ---
    ...
    if {"dc_name", "status", "sale_price"}.issubset(lost_df.columns):
        ...
        fig_dc.update_layout(xaxis_title="Distribution Center", yaxis_title="Lost Revenue ($)")
        st.plotly_chart(fig_dc, use_container_width=True)
    else:
        st.info("Missing one of: `dc_name`, `status`, `sale_price` – cannot plot DC breakdown.")

    # --- Category breakdown (bar or treemap) ---
    ...
    if {"category", "sale_price"}.issubset(lost_df.columns):
        ...
        if view_type == "Bar Chart":
            ...
            fig_cat.update_layout(xaxis_title="Category", yaxis_title="Lost Revenue ($)")
        ...
        st.info("Missing `category` or `sale_price` – cannot plot category breakdown.")

    # --- Detail Table: Top leaking orders ---
